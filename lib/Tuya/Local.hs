...
sendCmd' :: ToJSON a => ByteString -> Client -> CommandType -> a -> IO ()
sendCmd' key c cmd payload = do
  seqno <- atomicModifyIORef' (clientSequenceNumber c) (\x -> (x + 1, x))
  let msg = Msg seqno cmd 0 payload
  send c (encode (clientProtocol c) key msg)
...
recvBS' :: ByteString -> Int -> Client -> IO (Either String (Msg ByteString))
recvBS' key wait c = do
  mp <- recv wait c 65536
  case mp of
    Nothing -> return $ Left "recv timeout"
    Just p -> return $ decode (clientProtocol c) key p
...
recvMsg' :: FromJSON a => ByteString -> Int -> Client -> IO (Either String (Msg a))
recvMsg' key wait c = do
  bs <- recvBS' key wait c
  return $
    case bs of
      Right m ->
        case eitherDecodeStrict' (msgPayload m) of
